...
func TestReleaseLinksService_ListReleaseLinks(t *testing.T) {
	...
	releaseLinks, _, err := client.ReleaseLinks.ListReleaseLinks(1, "v0.1", &ListReleaseLinksOptions{})
	if err != nil {
		t.Error(err)
	}
	...
	if releaseLinks[0].Name != "awesome-v0.2.msi" {
		t.Errorf("release link name, expected '%s', got '%s'", "awesome-v0.2.msi", releaseLinks[0].Name)
	}
}
...
func TestReleaseLinksService_CreateReleaseLink(t *testing.T) {
	...
	if releaseLink.Name != "awesome-v0.2.dmg" {
		t.Errorf("release link name, expected '%s', got '%s'", "awesome-v0.2.dmg", releaseLink.Name)
	}
}
...
func TestReleaseLinksService_UpdateReleaseLink(t *testing.T) {
	...
	releaseLink, _, err := client.ReleaseLinks.UpdateReleaseLink(1, "v0.1", 1, &UpdateReleaseLinkOptions{
		Name: "awesome-v0.2.dmg",
	})
	if err != nil {
		t.Error(err)
